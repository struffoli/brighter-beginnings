...
async function getScholarships(search, sort, filter) {
  ...
  try {
    ...
    let filters = [null, "merit_based", "need_based", "essay_based", "nationwide"];
    const response = await fetch(
      "https://api.brighterbeginnings.me/scholarships" + (search === "" ? "?" : "?search=" + search + "&") + (sorts[sort] ? "sort=" + sorts[sort] + "&" : "") + (filters[filter] ? "filter=" + filters[filter] + "&" : "")); // also need to paginate using api instead
...
const Scholarships = () => {
  ...
  let filters = ["None", "Merit based", "Need based", "Essay based", "Nationwide"];

  const handleScholarshipsSearch = (searchText, sort, filter) => {
    getScholarships(searchText, sorts.indexOf(sort), filters.indexOf(filter)).then((data) => setApiScholarships(data));
  };
  ...
  return (
    <div>
      ...
      <AwesomeSearch handleSearch={handleScholarshipsSearch} sorts={sorts} filters={filters}/>
