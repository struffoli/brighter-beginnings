...
async function getOrganizations(search, sort, filter) {
  ...
  try {
    ...
    const response = await fetch(
      "https://api.brighterbeginnings.me/organizations" + (search === "" ? "?" : "?search=" + search + "&") + (sorts[sort] ? "sort=" + sorts[sort] + "&" : "") + (filters[filter] ? "filter=" + filters[filter] + "&" : "")); // also need to paginate using api instead
...
const Organizations = () => {
  ...
  const handleOrganizationsSearch = (searchText, sort, filter) => {
    getOrganizations(searchText, sorts.indexOf(sort), filters.indexOf(filter)).then((data) => setApiOrganizations(data));
  };
  ...
  return (
    <div>
      ...
      <AwesomeSearch handleSearch={handleOrganizationsSearch} sorts={sorts} filters={filters} />
